...
agg = pd.DataFrame({'Month' : ['Jan','Feb','Mar','Apr','May','June','July','Aug',
                    'Sep','Oct','Nov','Dec'],
                   'Male' : map(lambda x: count_gender_in_month(x, 'Male'), df.Month.unique()),
                   'Female' : map(lambda x: count_gender_in_month(x, 'Female'), df.Month.unique()),
                   ...
                   'Total' : map(lambda x: count_total_in_month(x,'total'), df.Month.unique())})
